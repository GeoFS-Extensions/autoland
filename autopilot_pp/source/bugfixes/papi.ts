...
define(["util"], function (util) {
  function papiBugfix() {
    ...
    var aircraft = window.geofs.aircraft.instance;

    function setPapi() {
      var collResult = window.geofs.getGroundAltitude(
        this.location[0],
        this.location[1]
      );
      ...
      var distance = window.geofs.utils.llaDistanceInMeters(
        relativeAicraftLla,
        this.location,
        this.location
      );
    ...
    Object.keys(window.geofs.fx.litRunways).forEach(function (id) {
      var runway = window.geofs.fx.litRunways[id];
