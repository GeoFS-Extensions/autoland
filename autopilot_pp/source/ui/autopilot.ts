...
define([
  ...
  "getwaypoint",
  "shouldntHaveAp",
], function (ko, gc, ap, util, getWaypoint, shouldntHaveAp: string[]) {
  ...
  function AutopilotVM() {
    ...
    this.toggle = () => {
      if (!shouldntHaveAp.includes(window.geofs.aircraft.instance.id)) {
        ap.toggle();
      }
    };

    // Make it so when changing to an aircraft in the array the autopilot turns off:
    const oldChange = window.geofs.aircraft.Aircraft.change;
    window.geofs.aircraft.Aircraft.change = (a, b, c, d) => {
