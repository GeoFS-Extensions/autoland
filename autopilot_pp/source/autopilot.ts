...
define([
  ...
  var on = ko.pureComputed({
    ...
    write: function (newValue) {
      ...
      if (window.window.geofs.aircraft.instance.setup.autopilot && newValue)
        _on(true);
      else _on(false);
    },
  });
  ...
  on.subscribe(function (newValue) {
    ...
    window.window.controls.autopilot.on = newValue;

    // Toggle the autopilot indicator provided by GEFS.
    window.ui.hud.autopilotIndicator(newValue);
  ...
  apModes.heading.enabled.subscribe(function (newValue) {
    if (newValue) pidControls.roll.init(window.controls.roll);
  });

  apModes.altitude.enabled.subscribe(function (newValue) {
    if (newValue) {
      pidControls.climb.init(
        window.geofs.aircraft.instance.animationValue.atilt
      );
      // We don't want to set pitch outside the range for zero elevator trim.
      pidControls.pitch.init(util.clamp(window.controls.pitch, -1, 1));
  ...
  apModes.heading.enabled.subscribe(function (newValue) {
    if (newValue) pidControls.throttle.init(window.controls.throttle);
  });
  ...
  function update(dt) {
    ...
    function updateHeading() {
      ...
      var deltaHeading = util.fixAngle(
      ...
      var targetBankAngle = util.clamp(
        deltaHeading,
        -maxBankAngle,
        maxBankAngle
      );
      ...
      window.controls.roll = util.exponentialSmoothing(
        "apRoll",
        result / speedRatio,
        0.9
      );
      ...
      window.controls.yaw = util.exponentialSmoothing(
        "apYaw",
      ...
      if (window.geofs.aircraft.instance.name === "a380")
    ...
    function updateAltitude() {
      ...
      window.controls.rawPitch = util.exponentialSmoothing(
        "apPitch",
        result / speedRatio,
        0.9
      );

      window.geofs.debug.watch("targetClimbrate", targetClimbRate);
      window.geofs.debug.watch("aTargetTilt", targetTilt);
    }
    ...
    function updateThrottle() {
      ...
      window.controls.throttle = util.clamp(
