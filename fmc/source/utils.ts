...
define(["debug", "exports"], function (debug, exports) {
  ...
  function getGroundSpeed() {
    var tas = window.geofs.aircraft.instance.animationValue.ktas;
    var vs =
      60 * window.geofs.aircraft.instance.animationValue.climbrate * FEET_TO_NM;
   ...
   */
  function getETE(d, a) {
    var hours = d / window.geofs.aircraft.instance.animationValue.ktas;
    var h = parseInt(hours.toString());
    var m = Math.round(60 * (hours - h));
    if (a)
      m += Math.round(
        window.geofs.aircraft.instance.animationValue.altitude / 4000
      );
